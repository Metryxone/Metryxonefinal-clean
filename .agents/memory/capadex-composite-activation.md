...
**Second trap (FIXED):** `seedCapadexSignals` (`capadex-signals-seeder.ts`) used to be **INSERT-only — it never ran CREATE TABLE**. On a DB where `capadex_signals` was never created by a prior migration, every INSERT threw, the error was caught + logged (non-fatal), `seededOnce` stayed false, and the ontology silently never existed → `loadCompositeRuntime` returned 0 definitions. Now `ensureSignalOntologySchema(pool)` runs `CREATE TABLE IF NOT EXISTS capadex_signals` (mirroring the Tier-3 shape in `migrations/20260528_signal_ontology_tables.sql`) at the top of the seed `try` block, so the ontology bootstraps itself on a fresh DB. Idempotent. The "41 rows / 22 clusters" figures above still depend on the table being seeded; verify `to_regclass('capadex_signals')` is non-null + row count before trusting composite-readiness claims.
