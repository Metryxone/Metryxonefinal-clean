...
## Gaps are closed by WIRING existing substrate, not building (durable)
All journey gaps here were closed by connecting EXISTING substrate (no new engine/table/DDL), each closure gated by `customerJourneyCompletion` so flag-OFF stays byte-identical. **Why:** "close the gap" means ENGINEERING closure (the journey path now exists end-to-end) — NOT proving real usage. Conflating the two is the dishonesty the contract forbids.
**How to apply — the closed-gap representation:** when open engineering gaps reach 0, set `JOURNEY_GAPS=[]` AND carry a SEPARATE `RESOLVED_JOURNEY_GAPS` array (`{id,title,closure,residual}`) + `summary.resolved_gap_count`. Every surface that reports gaps must expose BOTH (engine, scan `resolved_gaps`, generator gap-register, AND the `/api/admin/customer-journey/gaps` route — the route returning only the empty `gaps` is an API blind spot that contradicts the docs). The residual is always ADOPTION (real usage/outcome volume, usually 0) — a usage axis reported SEPARATELY (`/outcome-tail`), NEVER reclassified as a gap. Verdict stays `STRUCTURAL_COMPLETE_ADOPTION_PENDING`; closing engineering gaps does NOT promote it to outcome-verified.
**Re-run order is load-bearing:** edit registry/engine → `capadex-1.4-customer-journey-scan.ts` (re-measure, writes scan.json) → `capadex-1.4-generate-deliverables.ts` (reads ONLY scan.json). Generating before re-scanning makes the deliverables contradict the "closed" claims (scan-lock honesty).
