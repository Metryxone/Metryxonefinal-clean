...
**Observed (25 Jun 2026, Task #152):** a "re-run the seeds in prod" task was un-runnable two ways at once — prod was never deployed (`executeSql` env=production → `PRODUCTION_DATABASE_ERROR`) AND the task's premise ("ontology-seed carries the new curated roles on restart") was false: `runOntologySeed` (ontology-seed.ts) only fires behind `POST /api/ontology/overview/seed`, so the dedicated `ont_roles` ROLE_* rows (the O*NET-library side the employer match resolves a job title to) are NOT boot-seeded — a per-role activation seeder must `ensureLibraryRoles` (ON CONFLICT) itself before bridging. Cured with `services/role-bridge-activation.ts` (boot seeder mirroring role-library-expansion); directMatchCount rose 0→7 for both Backend/Sr Backend Engineer.

**Observed (1 Jul 2026, Task #421):** still no prod deploy — `executeSql` env=production → `PRODUCTION_DATABASE_ERROR` ("does not have a production Neon database"). The O*NET→Role-DNA estimated-weight bridge (`bridgeOnetDerivedWeights`) only ran on the slow on-demand O*NET import or the admin trigger — never at boot — so even a future prod deploy wouldn't populate `onto_role_weights.source='onet_derived'` without a manual click. Cured with the boot-seeder remedy below (added to `index.ts` after the role-bridge activation): `ensureOntoRoleWeightSourceColumn` → probe derived-row count → if 0 run the bridge, else no-op. Verified against the shared DB: 9→191 derived weights (75 comps, 14/15 roles), idempotent, warm restart no-ops, fresh (deleted→0) restart re-activates to 191.

**Remedy pattern — self-running startup seeder:** when the deliverable is "this data must exist in prod" and there is no deployment yet (or you can't reach prod), don't ship a CLI script nobody will run. Promote the seed into a **startup hook** that fires fire-and-forget at route-registration: a single source-of-truth service exporting (a) `ensure*Schema` mirroring the migration with `IF NOT EXISTS` DDL (the isolated env often lacks the migration since there's no migration runner — the ensure-schema IS the apply mechanism), (b) the seed itself **delete-then-insert by provenance** so it's idempotent, and (c) a guard that probes for the provenance rows and **no-ops once present** (verify: a 2nd restart logs nothing). Refactor any existing CLI scripts into thin wrappers calling the same service so there's no drift. Seed the **data unconditionally** (so publish alone activates it) only when it's observably inert flag-OFF (e.g. excluded by a default-region/baseFilter and behind a 503'd read route); the feature flag then gates only the read surface.
