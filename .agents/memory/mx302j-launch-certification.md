...
**Dev-first activation:** the A→I activation set is now **9** dev env flags (`FF_CAREER_LAUNCHPAD`,
`FF_LAUNCHPAD_DASHBOARD`, `FF_STUDENT_CAREER_BUILDER`, `FF_EMPLOYABILITY_STUDIO`,
`FF_INSTITUTIONAL_INTELLIGENCE`, `FF_ECOSYSTEM_COMMUNITY`, `FF_CAREER_DISCOVERY`,
`FF_CAMPUS_PLACEMENT`, `FF_LEARNING_PASSPORT_LOOP`) all in `.replit [userenv.development]`.
With all 9 ON, Activation now hits **9/9** (Phase C was backend-merged — see below — so it can
report `enabled:true`). Verdict still STRUCTURAL-READY / **PARTIAL** / not production-ready:
outcome still ABSTAINS (realized offers 0 < k_min=30) and adoption stayed 0 after the flip —
live proof that flags do NOT seed data. Set dev-scope only (prod stays OFF; reverse with
`deleteEnvVars` + Backend API restart).
