...
**Resolution / rule:** the bare `/api/ontology/{entity}` paths belong to the **admin taxonomy
CRUD (`ont_*`)**. The curated `onto_*` workforce reads live under `/api/ontology/curated/*`
(`industries/functions/subfunctions/role-families/roles` + `roles/:id/dna`). `DepartmentsPanel`
