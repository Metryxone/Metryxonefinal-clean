...
**Consequence (DERIVED → RESOLVED):** Phase 1.21 (Learning) and Phase 1.22 (Career) "almost entirely dormant" / "matching genome empty" findings were **materially wrong and have since been REGENERATED with exact `COUNT(*)`** (each carries a "⚠️ MEASUREMENT-INTEGRITY CORRECTION" block at its head). Phases 1.18–1.20 were re-verified with exact counts: **1.20 (RIE/Intervention) was also materially wrong and has been regenerated** (catalogs populated: `intervention_library`=213 etc., RIE has run); **1.18 (Pragati / Conversation) stands** (conversation + pragati runtime genuinely 0 by exact count); **1.19 (WC-3 / Decision) was corrected** (demo rows present, not all-0). Phase 1.17 used exact COUNT(*) and stands. **Durable fix:** all population verdicts use exact `COUNT(*)` — `.agents/memory/n-live-tup-stale-population-audit.md`.
