# Deliverable 2 — Lever 2: Outcome Trend
_Generated 2026-06-08T16:35:30.858Z_

Direction of **outcome progression** across each user's session history,
computed by the REUSED longitudinal trend math over existing state (`wc3_outcome_state.current_order`),
...
|---|---|---|
| Coverage | **0/2 (0.0%)** | eligible users with a outcome trend row |
...
## Source state
- `wc3_outcome_state.current_order` — table has **0 rows (0 rows — outcome state never persisted for any session)**.

## Per-user outcome trends
