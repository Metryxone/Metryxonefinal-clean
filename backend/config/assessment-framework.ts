...
export const SPEC_19_CROSSWALK: { specName: string; canonicalKey: string; note: string }[] = [
  ...
  { specName: 'Progress', canonicalKey: 'progress', note: 'Direct — systematic re-measurement now instrumented via the progression-capture hook (reuse).' },
  ...
  { specName: 'Exit', canonicalKey: 'exit', note: 'Close-the-loop exit hook implemented via reuse (reached-Mastery eligibility + reached_mastery milestone capture); adoption-gated.' },
  { specName: 'Continuous', canonicalKey: 'continuous', note: 'Interval re-administration via a derived freshness signal (reuse); no server cron; adoption-gated.' },
];
