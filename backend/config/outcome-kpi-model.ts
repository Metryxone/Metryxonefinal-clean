...
export const KPI_FAMILIES: KpiFamily[] = [
  ...
  { key: 'ai', label: 'AI KPIs', definition: 'Diagnosis coverage + recommendation acceptance / effectiveness.', exampleKpis: ['Diagnosis coverage', 'Recommendation acceptance (honest-null)', 'AI effectiveness (abstained)'], status: 'PARTIAL', statusNote: 'Diagnosis/recommendation substrate present; acceptance stays honest-null (no per-recommendation acceptance signal) and effectiveness is WIRED via REUSE of the validation-loop calibration mechanism but ABSTAINED (null) until ≥ k_min real prediction+outcome pairs accrue (Confidence axis, never fabricated).', evidence: { services: ['services/recommendation-intelligence-engine.ts'], tables: ['development_recommendations'] } },
...
export const OUTCOME_MODEL: OutcomePath[] = [
  ...
  },
  {
    ...
    status: 'PARTIAL',
    statusNote: 'Reaches the FULL loop incl. reassessment→improvement→measured-outcome→KPI as a MECHANISM, but promotion is DERIVED (not uniformly gated — a progression-engine residual) and validation is ADOPTION-pending (Coverage⟂Adoption⟂Confidence).',
    evidence: {
