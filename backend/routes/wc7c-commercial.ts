...
import {
  ...
  isCommercialRecurringRevenueEnabled,
} from '../config/feature-flags';
import { buildRevenueIntelligence, buildRecurringRevenue } from '../services/wc7c/revenue-intelligence';
...
export function registerWc7cCommercialRoutes(
  ...
  // Flag `commercialRecurringRevenue` (default OFF → {enabled:false}, the existing
  // revenue-intelligence route above is untouched → byte-identical legacy). Read-only; never 500s.
  app.get(
    '/api/capadex/admin/recurring-revenue',
    requireAuth,
    requireSuperAdmin,
    async (_req: Request, res: Response) => {
      if (!isCommercialRecurringRevenueEnabled()) {
        return res.status(200).json({ enabled: false });
      }
      try {
        const data = await buildRecurringRevenue(pool);
        return res.status(200).json({ enabled: true, ...data });
      } catch (err) {
        console.warn(
          '[wc7c-commercial] recurring-revenue failed, degrading:',
          err instanceof Error ? err.message : String(err),
        );
        return res.status(200).json({ enabled: true, degraded: true, reason: 'recurring_revenue_error' });
      }
    },
  );
