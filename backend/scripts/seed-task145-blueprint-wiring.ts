/**
 * Task #145 — blueprint wiring (lever 2b) for Backend / Senior Backend Engineer.
 *
 * Adding a competency to a role's DNA (already present in onto_role_weights) and
 * authoring questions (seed-task145-competency-questions.ts) is not enough:
 ...
 *
 * This script closes that gap for role_be_eng / role_sr_be_eng only, additively
 * and reversibly:
 *   1. Reuses the role's existing canonical blueprint (bp_*_v1 preferred);
 *      creates `bp_<role>_t145` only if none exists.
 ...
 * Idempotent (NOT EXISTS); reversible (created blueprints carry source='t145',
 * synced comp rows carry source='t145_sync'). Run with --apply to write.
 */
import { Pool } from 'pg';

const APPLY = process.argv.includes('--apply');
const TARGET_ROLES = ['role_be_eng', 'role_sr_be_eng'];
