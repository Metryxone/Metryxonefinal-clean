/**
 * Task #145 — end-to-end proof (transient; cleans up after itself).
 *
 * Proves that bridging role_be_eng / role_sr_be_eng to dedicated curated ont_roles
...
const CASES: { roleId: string; jobTitle: string }[] = [
  { roleId: 'role_be_eng', jobTitle: 'Backend Engineer' },
  { roleId: 'role_sr_be_eng', jobTitle: 'Senior Backend Engineer' },
];
...
async function run(pool: Pool, roleId: string, jobTitle: string) {
  const email = `t145_${roleId}@example.com`;
  const candidate = { email };
  const job = { id: `t145_job_${roleId}`, title: jobTitle };
