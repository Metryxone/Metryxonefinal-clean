...
const ASSET_SPECS: AssetSpec[] = [
  ...
  { key: 'onto_role_competency_profiles', label: 'Role competency profiles (role → competency → level/weight/criticality · Phase 1.5)', namespace: 'onto_*', table: 'onto_role_competency_profiles', category: 'competency_library', expectation: 'expected_populated', confidence: 'curated example profiles over existing roles + competencies; weights reported as-is (never auto-normalised)' },
  { key: 'onto_assessment_blueprints', label: 'Assessment blueprints (assessment-facing projection of role profiles · Phase 1.6)', namespace: 'onto_*', table: 'onto_assessment_blueprints', category: 'assessment', expectation: 'expected_populated', confidence: 'derived verbatim from role competency profiles; never redesigns assessment workflows' },
  { key: 'onto_blueprint_competency_map', label: 'Blueprint → competency relationships (level/weight/criticality · Phase 1.6)', namespace: 'onto_*', table: 'onto_blueprint_competency_map', category: 'assessment', expectation: 'expected_populated', confidence: 'inherited verbatim from the source role competency profile; weights reported as-is' },
  { key: 'onto_role_assessment_map', label: 'Role → assessment blueprint mapping (Phase 1.6)', namespace: 'onto_*', table: 'onto_role_assessment_map', category: 'assessment', expectation: 'expected_populated', confidence: 'each profiled role mapped to its derived blueprint (primary)' },
  { key: 'onto_competency_question_map', label: 'Competency → question mapping (Phase 1.6)', namespace: 'onto_*', table: 'onto_competency_question_map', category: 'assessment', expectation: 'known_gap_pending_import', confidence: 'infrastructure only; derived from competency_question_templates — honestly empty until questions are curated (never fabricated)' },
