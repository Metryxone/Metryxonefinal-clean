/**
 * Phase 3.6 — Industry Gap Engine.
 *
 * PURE, deterministic, never-throws extraction of the gap areas between a
 * subject and an INDUSTRY's aggregated competency demand. Developmental signal
 * only — never a hiring/placement verdict.
 *
 * Composes (never recomputes) the industry ReadinessResult. Surfaces the gap
 * areas, the critical (blocking) gaps, and the single most material (top) gap so
 * a development plan can be anchored on it. Mirrors the role-level RoleGap shape.
 *
 * Honesty contract:
...
export const INDUSTRY_GAP_ENGINE_VERSION = 'phase-3.6';

export interface IndustryGapTop {
...
export interface IndustryGap {
  top_gap: IndustryGapTop | null;
  gap_areas: ReadinessGap[];
  critical_gaps: ReadinessGap[];
  blocking_gaps: number;
}

function pickTopGap(readiness: ReadinessResult): IndustryGapTop | null {
...
/**
 * Extract the industry gap view from a readiness result. `readiness` may be null
 * (no derivable requirements / subject unscored) → an empty, honest gap view.
 */
export function assessIndustryGaps(readiness: ReadinessResult | null | undefined): IndustryGap {
