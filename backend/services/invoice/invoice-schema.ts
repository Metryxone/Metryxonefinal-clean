/**
 * Task #6 — Invoice & GST Engine · lazy ensure-schema.
 *
 * Mirrors migration `migrations/20260617_invoice_gst_engine.sql` EXACTLY. There is no migration
 ...
 *
 * ADDITIVE ONLY: `inv_*` namespace, alongside (never touching) capadex_payments / comm_*. Called
 * ONLY when the invoiceGstEngine flag is ON, so flag-OFF leaves the schema byte-identical to legacy.
 */
...
export function ensureInvoiceSchema(pool: Pool): Promise<void> {
  if (!schemaPromise) {
