...
class User(Base, UUIDPKMixin, TimestampMixin):
    ...
    student: Mapped[Optional["Student"]] = relationship(back_populates="user", cascade="all, delete-orphan", uselist=False)
...
class QuestionBank(Base, UUIDPKMixin, TimestampMixin, ExtraJSONMixin):
    ...
    test_links: Mapped[List["TestQuestion"]] = relationship(back_populates="question", cascade="all, delete-orphan")
...
class Test(Base, UUIDPKMixin, TimestampMixin, ExtraJSONMixin):
    __tablename__ = "tests"
    ...
    status: Mapped[TestStatus] = mapped_column(Enum(TestStatus, name="test_status"), default=TestStatus.DRAFT, nullable=False)
    ...
    __table_args__ = (Index("ix_tests_status", "status"),)
...
class TestQuestion(Base):
    ...
    question_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("question_bank.id", ondelete="CASCADE"), primary_key=True
    )

    sequence: Mapped[int] = mapped_column(Integer, nullable=False)

    test: Mapped["Test"] = relationship(back_populates="questions")
    question: Mapped["QuestionBank"] = relationship(back_populates="test_links")

    __table_args__ = (
        UniqueConstraint("test_id", "sequence", name="uq_test_questions_testid_sequence"),
        Index("ix_test_questions_test_id", "test_id"),
    )


class Attempt(Base, UUIDPKMixin, TimestampMixin, ExtraJSONMixin):
