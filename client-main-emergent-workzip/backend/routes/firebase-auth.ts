      ...
      try {
        ...
        if (!projectId) {
          return res.status(500).json({
            message:
              "Firebase project ID not configured on the server (VITE_FIREBASE_PROJECT_ID).",
          });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!projectId) {
          return res
            .status(500)
            .json({ message: "Firebase project ID not configured." });
        }
        ...
        req.login(sessionUser, (err) => {
          ...
          return res.json({ user: sessionUser, isNewUser: true });
        });
      } catch (err) {
        const msg = err instanceof Error ? err.message : "Unknown error";
        return res
