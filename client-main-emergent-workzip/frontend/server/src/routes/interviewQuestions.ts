...
import { interviewQuestionBank } from '../db/schema.js';
...
function getToken(req: Request): any {
  try {
    ...
    return t ? JSON.parse(Buffer.from(t.split('.')[1], 'base64').toString()) : null;
  } catch { return null; }
}
function isAdmin(req: Request): boolean {
  const p = getToken(req);
  return p && ['superadmin', 'admin', 'hr'].includes(p.role || '');
}

// ── GET /api/interview-questions — list with filters ─────────────────────────
router.get('/', async (req: Request, res: Response) => {
  try {
    const { industry, role, level, category, difficulty, search, active, limit = '200', offset = '0' } = req.query as Record<string, string>;

    let query = db.select().from(interviewQuestionBank);
    const conditions: any[] = [];

    if (active !== 'false') conditions.push(eq(interviewQuestionBank.isActive, true));
    if (industry && industry !== 'All') conditions.push(or(eq(interviewQuestionBank.industry, industry), eq(interviewQuestionBank.industry, 'General'))!);
    if (role && role !== 'All') conditions.push(or(eq(interviewQuestionBank.role, role), eq(interviewQuestionBank.role, 'General'))!);
    if (level && level !== 'All') conditions.push(or(eq(interviewQuestionBank.positionLevel, level), eq(interviewQuestionBank.positionLevel, 'Any'))!);
    if (category && category !== 'All') conditions.push(eq(interviewQuestionBank.category, category));
    if (difficulty && difficulty !== 'All') conditions.push(eq(interviewQuestionBank.difficulty, difficulty));
    if (search) conditions.push(ilike(interviewQuestionBank.question, `%${search}%`));

    const rows = await db.select().from(interviewQuestionBank)
      .where(conditions.length > 0 ? and(...conditions) : undefined)
      .orderBy(asc(interviewQuestionBank.category), asc(interviewQuestionBank.question))
      .limit(parseInt(limit))
      .offset(parseInt(offset));

    res.json({ success: true, questions: rows, total: rows.length });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
});

// ── GET /api/interview-questions/stats ────────────────────────────────────────
router.get('/stats', async (_req: Request, res: Response) => {
  try {
    const all = await db.select().from(interviewQuestionBank);
    const byCategory = all.reduce((acc: Record<string, number>, q) => {
      acc[q.category] = (acc[q.category] || 0) + 1;
      return acc;
    }, {});
    const byIndustry = all.reduce((acc: Record<string, number>, q) => {
      acc[q.industry] = (acc[q.industry] || 0) + 1;
      return acc;
    }, {});
    const byLevel = all.reduce((acc: Record<string, number>, q) => {
      acc[q.positionLevel] = (acc[q.positionLevel] || 0) + 1;
      return acc;
    }, {});
    res.json({
      success: true,
      ...
      byIndustry,
      byLevel,
      industries: [...new Set(all.map(q => q.industry))].sort(),
      roles: [...new Set(all.map(q => q.role))].sort(),
      categories: [...new Set(all.map(q => q.category))].sort(),
      levels: [...new Set(all.map(q => q.positionLevel))].sort(),
    });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const rows = await db.select().from(interviewQuestionBank).where(eq(interviewQuestionBank.id, req.params.id));
...
// ── POST /api/interview-questions — create (admin) ───────────────────────────
router.post('/', async (req: Request, res: Response) => {
  if (!isAdmin(req)) return res.status(403).json({ success: false, error: 'Forbidden' });
  try {
    const { question, expectedResponse, scoringCriteria, category, industry, role, positionLevel, difficulty, tags } = req.body;
    if (!question) return res.status(400).json({ success: false, error: 'question is required' });
    const token = getToken(req);
    const rows = await db.insert(interviewQuestionBank).values({
      question,
      ...
      role: role || 'General',
      positionLevel: positionLevel || 'Any',
      difficulty: difficulty || 'Medium',
      isActive: true,
      tags: tags || [],
      createdBy: token?.email || token?.username || 'admin',
    }).returning();
    res.json({ success: true, question: rows[0] });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
...
// ── PUT /api/interview-questions/:id — update (admin) ────────────────────────
router.put('/:id', async (req: Request, res: Response) => {
  if (!isAdmin(req)) return res.status(403).json({ success: false, error: 'Forbidden' });
  try {
    const { question, expectedResponse, scoringCriteria, category, industry, role, positionLevel, difficulty, isActive, tags } = req.body;
    const rows = await db.update(interviewQuestionBank)
      .set({
      ...
      .where(eq(interviewQuestionBank.id, req.params.id))
      .returning();
    if (!rows.length) return res.status(404).json({ success: false, error: 'Not found' });
    res.json({ success: true, question: rows[0] });
  } catch (err: any) {
...
// ── DELETE /api/interview-questions/:id — delete (admin) ─────────────────────
router.delete('/:id', async (req: Request, res: Response) => {
  if (!isAdmin(req)) return res.status(403).json({ success: false, error: 'Forbidden' });
  try {
    await db.delete(interviewQuestionBank).where(eq(interviewQuestionBank.id, req.params.id));
...
// ── POST /api/interview-questions/bulk-seed — re-seed defaults (superadmin) ──
router.post('/bulk-seed', async (req: Request, res: Response) => {
  if (!isAdmin(req)) return res.status(403).json({ success: false, error: 'Forbidden' });
  try {
    const count = await seedInterviewQuestions();
    res.json({ success: true, message: `Seeded ${count} questions` });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
// ─── Seed function (exported for use at startup) ───────────────────────────────

export async function seedInterviewQuestions(): Promise<number> {
  const existing = await db.select({ id: interviewQuestionBank.id }).from(interviewQuestionBank).limit(1);
  if (existing.length > 0) return 0; // already seeded

  const questions = getDefaultQuestions();
  await db.insert(interviewQuestionBank).values(questions);
  return questions.length;
}

function getDefaultQuestions() {
  const now = new Date();
  const q = (question: string, expectedResponse: string, scoringCriteria: string, category: string, industry: string, role: string, positionLevel: string, difficulty: string, tags: string[] = []) => ({
    question, expectedResponse, scoringCriteria, category, industry, role, positionLevel, difficulty,
    isActive: true, tags, createdBy: 'system', createdAt: now, updatedAt: now,
  });
