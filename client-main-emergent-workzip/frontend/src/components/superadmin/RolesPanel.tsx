...
import { Plus, Edit, Trash2, Search, Users, RefreshCw, Check, X, ChevronDown } from 'lucide-react';
              ...
              <TableBody>
                ...
                {filteredRoles.map(r => (
                  <TableRow key={r.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="text-right"><Button variant="ghost" size="sm" onClick={() => openEditRole(r)}><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => setDelConfirm({ id: r.id, name: r.title, type: 'role' })} className="text-red-500"><Trash2 className="h-4 w-4" /></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
