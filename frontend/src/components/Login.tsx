    ...
    if (provider === 'google') {
      ...
      try {
        ...
        if (res.ok && data.isNewUser && data.googleProfile) {
          // New user — sync their Google profile to Firestore then forward to registration
          ...
          // Existing user — sync updated profile snapshot to Firestore (idempotent merge)
          await syncProfileToFirestore(firebaseUser.uid, {
            email: firebaseUser.email ?? '',
            fullName: firebaseUser.displayName ?? data.user?.fullName ?? '',
