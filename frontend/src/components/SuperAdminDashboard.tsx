...
const ModuleAccessPanel = lazy(() => import('./superadmin/ModuleAccessPanel'));
...
export default function SuperAdminDashboard({ onNavigate }: { onNavigate?: (screen: Screen) => void }) {
  ...
  // ── Competency Coverage Matrices flag probe (file-registry flag competencyCoverageMatrices).
  //    Flag OFF → /feature-flag returns 503 → the tab is omitted (byte-identical UI). ──
  const { data: competencyCoverageMatricesEnabled = false } = useQuery<boolean>({
  ...
  const { data: platformLifecycleOperationsEnabled = false } = useQuery<boolean>({
  ...
  // ── MX-800 Phase 2.13 Enterprise Intelligence Integration Platform (flag enterpriseIntelligenceIntegration).
  //    Probe /feature-flag (res.ok) → flag OFF hides the tab entirely (byte-identical UI). ──
  const { data: enterpriseIntelligenceIntegrationEnabled = false } = useQuery<boolean>({
    queryKey: ['/api/admin/enterprise-intelligence-integration/feature-flag', 'enabled'],
    queryFn: async () => {
      const res = await fetch('/api/admin/enterprise-intelligence-integration/feature-flag', { credentials: 'include' });
      return res.ok;
    },
    enabled: isAuthenticated,
  });
  ...
  // ── Enterprise Certification (MX-105X) flag probe (file-registry flag enterpriseCertification).
  //    Flag OFF → /enabled returns 503 → tab omitted (byte-identical UI). ──
  const { data: enterpriseCertificationEnabled = false } = useQuery<boolean>({
  ...
  // ── Enterprise Assessment Builder (CAPADEX 3.0 · 3.3) flag probe (file-registry flag assessmentBuilder).
  //    Probe /api/assessment-builder/enabled (503-before-auth when OFF) → tab omitted (byte-identical UI). ──
  const { data: assessmentBuilderEnabled = false } = useQuery<boolean>({
    queryKey: ['/api/assessment-builder/enabled', 'enabled'],
    queryFn: async () => {
      const res = await fetch('/api/assessment-builder/enabled', { credentials: 'include' });
      return res.ok;
    },
    enabled: isAuthenticated,
  });

  // ── Enterprise Assessment Delivery (CAPADEX 3.0 · 3.4) flag probe (file-registry flag assessmentDelivery).
  //    Probe /api/assessment-delivery/enabled (503-before-auth when OFF) → tab omitted (byte-identical UI). ──
  const { data: assessmentDeliveryEnabled = false } = useQuery<boolean>({
  ...
  // ── Score Standardization (CAPADEX 3.0 · 3.8) flag probe (file-registry flag scoreStandardization).
  //    Probe /api/score-standardization/enabled (503-before-auth when OFF) → tab omitted (byte-identical UI). ──
  const { data: scoreStandardizationEnabled = false } = useQuery<boolean>({
        ...
        <div className="p-6">
          {statsLoading ? (
            ...
            <DialogsErrorBoundary resetKey={activeTab}>
              ...
              {activeTab === 'module-access'    && <ModuleAccessPanel />}
