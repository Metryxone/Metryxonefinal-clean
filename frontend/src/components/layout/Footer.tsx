...
// Maps footer link slugs to valid Screen values (null = no matching screen yet)
const SLUG_TO_SCREEN: Record<string, Screen | null> = {
  "lbi":                "lbi-product",
  "exam-ready":         "exam-ready",
  "competency":         "competency-intelligence",
  "metryx-ai":          "metryxai-assistant",
  "analytics":          "benchmark-dashboard",
  "schools":            "k12-schools",
  "campus":             "campus-recruit",
              ...
              <div key={section.key} data-testid={`footer-section-${section.key}`}>
                ...
                <ul className="space-y-2">
                  {section.links.map((link) => {
                    const target = SLUG_TO_SCREEN[link.slug];
                    return (
                      <li key={link.slug}>
                        <button
                          onClick={() => target && onNavigate?.(target)}
                          className="text-[11px] leading-snug transition-colors hover:text-[var(--accent-cyan)] flex items-start gap-1 group text-left"
                          style={{
        ...
        >
          ...
          <div className="flex gap-4">
            {[
              ...
              const target = SLUG_TO_SCREEN[l.slug];
              return (
                <button
                  key={l.slug}
                  onClick={() => target && onNavigate?.(target)}
                  className="transition-colors hover:text-[var(--accent-cyan)]"
                  style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "'Inter', sans-serif", fontSize: "10px", color: "var(--text-muted)" }}
