...
const BASE = '/api/admin/assessment-builder';
...
export default function AssessmentBuilderPanel() {
  const { data, isLoading, isError, refetch, isFetching } = useQuery<{
    ...
    queryFn: async () => {
      const probe = await fetch(`/api/assessment-builder/enabled`, { credentials: 'include' });
      ...
      if (!probe.ok) throw new Error(`assessment-builder enabled probe failed: ${probe.status}`);
  ...
  if (isLoading) return <div className="p-6 text-sm text-muted-foreground">Loading Enterprise Assessment Builder certification…</div>;
  if (isError || !data) {
    return (
      <div className="p-6">
        <div className="flex items-center gap-2 text-sm text-red-600"><AlertTriangle className="h-4 w-4" /> Failed to load Enterprise Assessment Builder certification.</div>
    ...
    return (
      <div className="p-6">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <CircleSlash className="h-4 w-4" /> Enterprise Assessment Builder is disabled (flag <code>assessmentBuilder</code> OFF).
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-5 p-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold" style={{ color: BRAND.primary }}>Enterprise Assessment Builder</h2>
            <p className="max-w-3xl text-xs text-muted-foreground">
              Read-only certification of the ONE canonical AUTHORING platform COMPOSING the existing blueprint/assembly
              services under one registry + additive <code>ab_*</code> overlay. Scope is design · compose · configure ·
              validate · version · approve · publish — NOT deliver/score/psychometrics. The SEVEN dimensions
              (builder · blueprint · validation · version_management · publishing · apis · frontend) are reported
              SEPARATELY — never composited. Adoption is a SEPARATE usage axis, never a gap. null = not measurable
      ...
      <Section title="Seven certification dimensions (never composited)" icon={<Layers className="h-4 w-4" style={{ color: BRAND.primary }} />}
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <Stat label="Designer actions" value={s.designer_actions?.count} hint={statusLine(s.designer_actions?.status_counts)} />
          <Stat label="Structure levels" value={s.structure_levels?.count} hint={statusLine(s.structure_levels?.status_counts)} />
          <Stat label="Composition caps" value={s.composition?.count} hint={statusLine(s.composition?.status_counts)} />
          <Stat label="Templates" value={s.templates?.count} hint={statusLine(s.templates?.status_counts)} />
          <Stat label="Blueprint caps" value={s.blueprint?.capability_count} hint={statusLine(s.blueprint?.status_counts)} />
          <Stat label="Rule types" value={s.rules?.rule_count} hint={statusLine(s.rules?.status_counts)} />
          <Stat label="Config options" value={s.config?.option_count} hint={statusLine(s.config?.status_counts)} />
          <Stat label="Version mgmt" value={s.version_management?.capability_count} hint={statusLine(s.version_management?.status_counts)} />
          <Stat label="Validation checks" value={s.validation?.check_count} hint={statusLine(s.validation?.status_counts)} />
          <Stat label="Workflow states" value={s.workflow?.state_count} hint={statusLine(s.workflow?.status_counts)} />
      ...
      <Section title="Adoption — real authored-assessment volume (SEPARATE axis, never a gap)" icon={<Boxes className="h-4 w-4" style={{ color: BRAND.primary }} />}
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
          <Stat label="Assessments" value={ado.assessments?.assessments} hint={`published ${ado.assessments?.published ?? '—'} · owned ${ado.assessments?.owned ?? '—'}`} />
          <Stat label="Versioned assessments" value={ado.versions?.versioned_assessments} hint={`${ado.versions?.total_versions ?? '—'} versions · ${ado.versions?.drafts ?? '—'} drafts`} />
          <Stat label="Blueprints" value={ado.blueprints?.blueprints} hint={`bound ${ado.blueprints?.bound ?? '—'}`} />
          <Stat label="Templates" value={ado.templates?.templates} hint={`${ado.templates?.categories ?? '—'} categories`} />
          <Stat label="Validation runs" value={ado.validation?.runs} hint={`passed ${ado.validation?.passed ?? '—'} · assessments ${ado.validation?.assessments_validated ?? '—'}`} />
