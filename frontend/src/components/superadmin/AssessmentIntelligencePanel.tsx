...
import { RefreshCw, AlertTriangle, CircleSlash, Boxes, Layers, ShieldCheck, GitBranch, ListChecks, Brain, Sigma } from 'lucide-react';
import { Button } from '../ui/button';
import { Badge } from '../ui/badge';
import InterpretationWorkbench from '../intelligence/InterpretationWorkbench';
...
export default function AssessmentIntelligencePanel() {
  const { data, isLoading, isError, refetch, isFetching } = useQuery<{
    ...
    queryFn: async () => {
      const probe = await fetch(`/api/assessment-intelligence/enabled`, { credentials: 'include' });
      ...
      if (!probe.ok) throw new Error(`assessment-intelligence enabled probe failed: ${probe.status}`);
  ...
  if (isLoading) return <div className="p-6 text-sm text-muted-foreground">Loading Assessment Intelligence certification…</div>;
  if (isError || !data) {
    return (
      <div className="p-6">
        <div className="flex items-center gap-2 text-sm text-red-600"><AlertTriangle className="h-4 w-4" /> Failed to load Assessment Intelligence certification.</div>
    ...
    return (
      <div className="p-6">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <CircleSlash className="h-4 w-4" /> Assessment Intelligence is disabled (flag <code>assessmentIntelligence</code> OFF).
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-5 p-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <Brain className="mt-0.5 h-6 w-6" style={{ color: BRAND.primary }} />
          <div>
            <h2 className="text-lg font-bold" style={{ color: BRAND.primary }}>Assessment Intelligence</h2>
            <p className="max-w-3xl text-xs text-muted-foreground">
              Read-only certification of the ONE canonical INTERPRETATION & REPORTING layer COMPOSING the existing
              interpretation services (psychometric-standardization · benchmark-engine · peer-benchmark ·
              intelligence-narrative-engine · ai-reasoning-engine · dynamic-report) under one registry + an additive
              <code>aint_*</code> overlay — NO duplicate interpretation / benchmark / narrative / report engine, NO V2.
              ...
              VALIDATED result (3.5 Scoring + 3.6 Science) into MEANING and NEVER re-scores or re-validates the
              instrument. The EIGHT dimensions are reported SEPARATELY — never composited. Adoption is a SEPARATE usage
              axis, never a gap. null = not measurable (never a fabricated 0). Norm-referenced statistics + benchmarks
              ABSTAIN below k_min real members; AI narrative confidence stays honestly null while cold-start.
            </p>
          </div>
        </div>
      ...
      <Section title="Eight certification dimensions (never composited)" icon={<Layers className="h-4 w-4" style={{ color: BRAND.primary }} />}
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <Stat label="Norm types" value={s.norm_types?.count} hint={statusLine(s.norm_types?.status_counts)} />
          <Stat label="Standard-score types" value={s.standard_score_types?.count} hint={statusLine(s.standard_score_types?.status_counts)} />
          <Stat label="Benchmark scopes" value={s.benchmark_scopes?.count} hint={statusLine(s.benchmark_scopes?.status_counts)} />
          <Stat label="AI capabilities" value={s.ai_capabilities?.count} hint={statusLine(s.ai_capabilities?.status_counts)} />
          <Stat label="Report sections" value={s.report_sections?.count} hint={statusLine(s.report_sections?.status_counts)} />
          <Stat label="Performance metrics" value={s.performance_metrics?.count} hint={statusLine(s.performance_metrics?.status_counts)} />
      ...
      <Section title="Adoption — real interpreted / benchmarked / reported VOLUME (SEPARATE axis, never a gap)" icon={<Boxes className="h-4 w-4" style={{ color: BRAND.primary }} />}
        subtitle="Engineering closure ⟂ adoption. A dimension can be fully SUPPORTED while adoption is honestly 0. Norm-referenced statistics + benchmarks ABSTAIN below k_min real members. null (unreadable) ≠ 0 (empty).">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <Stat label="Norm tables" value={ado.norm_tables?.norm_tables} hint={`types ${ado.norm_tables?.types_used ?? '—'} · assessments ${ado.norm_tables?.assessments ?? '—'} · abstained ${ado.norm_tables?.abstained ?? '—'}`} />
          <Stat label="Standard scores" value={ado.standard_scores?.scores} hint={`subjects ${ado.standard_scores?.subjects ?? '—'} · assessments ${ado.standard_scores?.assessments ?? '—'} · abstained ${ado.standard_scores?.abstained ?? '—'}`} />
          <Stat label="Benchmarks" value={ado.benchmarks?.benchmarks} hint={`subjects ${ado.benchmarks?.subjects ?? '—'} · scopes ${ado.benchmarks?.scopes_used ?? '—'} · abstained ${ado.benchmarks?.abstained ?? '—'}`} />
          <Stat label="Interpretations" value={ado.interpretations?.interpretations} hint={`subjects ${ado.interpretations?.subjects ?? '—'} · w/conf ${ado.interpretations?.with_confidence ?? '—'} · abstained ${ado.interpretations?.abstained ?? '—'}`} />
          <Stat label="Reports" value={ado.reports?.reports} hint={`subjects ${ado.reports?.subjects ?? '—'} · assessments ${ado.reports?.assessments ?? '—'}`} />
          <Stat label="Performance" value={ado.performance?.performance} hint={`subjects ${ado.performance?.subjects ?? '—'} · assessments ${ado.performance?.assessments ?? '—'} · abstained ${ado.performance?.abstained ?? '—'}`} />
      ...
      <Section title="Interpretation workbench — standard scores · norm reference · benchmark · AI narrative · report · performance" icon={<Brain className="h-4 w-4" style={{ color: BRAND.primary }} />}
        subtitle="The pure, deterministic interpretation mechanisms that engineering-close the interpretation & reporting gaps via reuse-before-build. Norm-referenced statistics + benchmarks ABSTAIN below k_min real members — never fabricated. Interactive demos with sample content; persists nothing unless a write mechanism is called explicitly.">
