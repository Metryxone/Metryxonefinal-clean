                    ...
                    <CardHeader className="border-b">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>DPDP Act compliance tracking</CardDescription>
                        </div>
                        <div className="flex gap-2">
                          <Select value={statusFilter} onValueChange={setStatusFilter}>
                            <SelectTrigger className="w-36" data-testid="filter-consent-status">
                          ...
                          <Button variant="outline" onClick={() => refetchConsents()} data-testid="button-refresh-consents">
                    ...
                    <CardContent className="p-0">
                      <div className="divide-y">
                        {consents.map((consent: any) => (
                          <div key={consent.id} className="p-4 hover:bg-gray-50 transition-colors" data-testid={`consent-row-${consent.id}`}>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${BRAND.accent}15` }}>
                                  <ScrollText className="h-6 w-6" style={{ color: BRAND.accent }} />
                                </div>
                                <div>
                                  <div className="flex items-center gap-2">
                                    <p className="font-semibold capitalize" style={{ color: BRAND.primary }}>{(consent.consentType || '').replace(/_/g, ' ')}</p>
                                    {getStatusBadge(consent.status)}
                                  </div>
                                  <p className="text-sm text-gray-500">{consent.entityType} • Version {consent.consentVersion}</p>
                                </div>
                              </div>
                              <p className="text-sm text-gray-500">{formatDate(consent.grantedAt || consent.createdAt)}</p>
                            </div>
                          </div>
                        ))}
                        {consents.length === 0 && (
                          <div className="p-8 text-center text-gray-500">No consent records found</div>
                        )}
                      </div>
                    </CardContent>
                  </Card>
