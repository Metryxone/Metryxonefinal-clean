...
export default function FinancialsPanel() {
  ...
  // ── Invoice & GST Engine (Task #6) — flag-gated; /api/invoice/* return 503 when OFF. ──
  const [invOpen, setInvOpen] = useState(false);
  const [invDocType, setInvDocType] = useState('tax');
  const [invSourceType, setInvSourceType] = useState('capadex_payment');
  const [invSources, setInvSources] = useState<any[]>([]);
  const [invSourceId, setInvSourceId] = useState('');
  const [invBuyerGstin, setInvBuyerGstin] = useState('');
  const [invBuyerState, setInvBuyerState] = useState('');
  const [invNotes, setInvNotes] = useState('');
  const [invPreview, setInvPreview] = useState<any>(null);
  const [invGenerated, setInvGenerated] = useState<any>(null);
  const [invLoading, setInvLoading] = useState(false);
  const [invDisabled, setInvDisabled] = useState(false);

  const invIsRefundDoc = invDocType === 'refund_receipt' || invDocType === 'credit_note';

  const invResetResults = () => { setInvPreview(null); setInvGenerated(null); };

  const invLoadSources = useCallback(async (sourceType: string, docType: string) => {
    invResetResults();
    setInvSourceId('');
    setInvSources([]);
    try {
      const isRefund = docType === 'refund_receipt' || docType === 'credit_note';
      const effType = sourceType === 'comm_subscription' ? 'comm_subscription' : 'capadex_payment';
      const qs = new URLSearchParams({ type: effType });
      if (effType === 'capadex_payment') qs.set('status', isRefund ? 'refunded' : 'paid');
      const res = await apiRequest('GET', `/api/invoice/admin/sources?${qs.toString()}`);
      const data = await res.json();
      setInvSources(Array.isArray(data?.rows) ? data.rows : []);
      setInvDisabled(false);
    } catch (e: any) {
      if (String(e?.message || '').startsWith('503')) { setInvDisabled(true); }
      setInvSources([]);
    }
  }, []);

  const invOpenDialog = async () => {
    // Readiness check: when the invoiceGstEngine flag is OFF the API returns 503, and we preserve
    // the EXACT legacy stub behaviour (Coming Soon toast) — byte-identical to before the feature.
    try {
      await apiRequest('GET', '/api/invoice/admin/meta');
    } catch (e: any) {
      if (String(e?.message || '').startsWith('503')) {
        toast({ title: 'Coming Soon', description: 'Invoice generation will be available in the next release' });
        return;
      }
      // Any other error (e.g. transient network/auth) falls through so the admin sees the real dialog/error.
    }
    setInvOpen(true);
    setInvDisabled(false);
    invResetResults();
    invLoadSources(invSourceType, invDocType);
  };

  const invOnDocType = (v: string) => {
    setInvDocType(v);
    // Refund/credit documents require refunded-payment evidence; subscriptions carry none, so
    // force the source back to payments to match the backend gate.
    const isRefund = v === 'refund_receipt' || v === 'credit_note';
    const nextSource = isRefund && invSourceType === 'comm_subscription' ? 'capadex_payment' : invSourceType;
    if (nextSource !== invSourceType) setInvSourceType(nextSource);
    invLoadSources(nextSource, v);
  };
  const invOnSourceType = (v: string) => { setInvSourceType(v); invLoadSources(v, invDocType); };

  const invPreviewGst = async () => {
    const src = invSources.find((s) => String(s.id) === invSourceId);
    if (!src) { toast({ title: 'Select a source', description: 'Pick a real payment or subscription first.' }); return; }
    setInvLoading(true);
    try {
      const res = await apiRequest('POST', '/api/invoice/admin/gst-preview', {
        amount_paise: src.amount_paise ?? src.price_paise,
        buyer_gstin: invBuyerGstin || undefined,
        buyer_state_code: invBuyerState || undefined,
      });
      setInvPreview(await res.json());
    } catch (e: any) {
      toast({ title: 'Preview failed', description: String(e?.message || e), variant: 'destructive' });
    } finally { setInvLoading(false); }
  };

  const invGenerate = async () => {
    if (!invSourceId) { toast({ title: 'Select a source', description: 'Pick a real payment or subscription first.' }); return; }
    // GST-bearing documents (everything except a Proforma quote) need a determinable supply type.
    // Without a buyer state code or GSTIN the backend cannot compute the split and will abstain, so
    // guide the admin up-front rather than firing a request that is guaranteed to fail compliance.
    if (invDocType !== 'proforma' && !invBuyerState.trim() && !invBuyerGstin.trim()) {
      toast({
        title: 'GST context required',
        description: `A compliant ${invDocType.replace('_', ' ')} needs the buyer state code or a valid GSTIN so CGST/SGST vs IGST can be determined. Add one, or choose Proforma for a pre-tax quote.`,
        variant: 'destructive',
      });
      return;
    }
    setInvLoading(true);
    try {
      const res = await apiRequest('POST', '/api/invoice/admin/invoices', {
        doc_type: invDocType,
        source_type: invIsRefundDoc && invSourceType !== 'comm_subscription' ? 'refund' : invSourceType,
      ...
      const data = await res.json();
      setInvGenerated(data);
      toast({ title: 'Invoice generated', description: data?.invoice?.invoice_number || 'Document created.' });
    } catch (e: any) {
      toast({ title: 'Generation failed', description: String(e?.message || e), variant: 'destructive' });
    } finally { setInvLoading(false); }
  };

  const invDownloadPdf = async () => {
    if (!invGenerated?.invoice?.id) return;
    try {
      const res = await apiRequest('GET', `/api/invoice/admin/invoices/${invGenerated.invoice.id}/pdf`);
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${String(invGenerated.invoice.invoice_number).replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
  ...
  const invEmail = async () => {
    ...
    setInvLoading(true);
    try {
      const res = await apiRequest('POST', `/api/invoice/admin/invoices/${invGenerated.invoice.id}/email`, {});
      const data = await res.json();
      toast({
        title: data?.emailSent ? 'Invoice emailed' : 'Email not sent',
        description: data?.emailSent ? `Sent to ${data.to}` : 'Email delivery is not configured in this environment (no error — document is saved).',
      });
    } catch (e: any) {
      toast({ title: 'Email failed', description: String(e?.message || e), variant: 'destructive' });
    } finally { setInvLoading(false); }
  };

  const invMoney = (paise?: number) => paise == null ? '—' : new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', minimumFractionDigits: 2 }).format(Number(paise) / 100);
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button style={{ backgroundColor: BRAND.primary }} className="gap-2 text-white" data-testid="button-generate-invoice" onClick={invOpenDialog}><Receipt className="h-4 w-4" /> Generate Invoice</Button>
                    </div>
                  </div>
                  ...
                  <Dialog open={invOpen} onOpenChange={setInvOpen}>
                    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" data-testid="dialog-generate-invoice">
                      <DialogHeader>
                        <DialogTitle className="flex items-center gap-2"><Receipt className="h-5 w-5" style={{ color: BRAND.primary }} /> Generate Invoice / GST Document</DialogTitle>
                        <DialogDescription>Documents are built from real payments or subscriptions — never fabricated. GST is computed from seller &amp; buyer state codes.</DialogDescription>
                      </DialogHeader>

                      {invDisabled ? (
                        <div className="p-6 text-center">
                          <AlertCircle className="h-8 w-8 mx-auto mb-2 text-amber-500" />
                          <p className="font-medium">Invoice &amp; GST Engine is not enabled</p>
                          <p className="text-sm text-gray-500 mt-1">This feature is behind the <code>invoiceGstEngine</code> flag (default OFF). Enable it to generate documents.</p>
                        </div>
                      ) : (
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <Select value={invDocType} onValueChange={invOnDocType}>
                                <SelectTrigger data-testid="select-invoice-doctype"><SelectValue /></SelectTrigger>
                                <SelectContent>
                            ...
                            <div>
                              ...
                              <Select value={invSourceType} onValueChange={invOnSourceType}>
                                <SelectTrigger data-testid="select-invoice-sourcetype"><SelectValue /></SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="capadex_payment">CAPADEX payment</SelectItem>
                                  <SelectItem value="comm_subscription" disabled={invIsRefundDoc}>Subscription{invIsRefundDoc ? ' (n/a for refunds)' : ''}</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                          </div>
                          ...
                          <div>
                            <Label className="text-xs">Real source row {invIsRefundDoc && invSourceType !== 'comm_subscription' ? '(refunded payments only)' : ''}</Label>
                            <Select value={invSourceId} onValueChange={(v) => { setInvSourceId(v); invResetResults(); }}>
                              <SelectTrigger data-testid="select-invoice-source"><SelectValue placeholder={invSources.length ? 'Select a source…' : 'No matching source rows found'} /></SelectTrigger>
                              <SelectContent>
                                {invSources.map((s) => (
                                  <SelectItem key={s.id} value={String(s.id)}>
                                    {(s.customer_name || s.participant_name || s.customer_email || s.email || 'Customer')} · {invMoney(s.amount_paise ?? s.price_paise)} · {(s.stage_name || s.plan_name || s.concern_name || s.status || '')}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                            {invSources.length === 0 && (
                              <p className="text-xs text-gray-500 mt-1">No real source rows match this document type. Documents are never fabricated from empty data.</p>
                            )}
                          </div>

                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <Label className="text-xs">Buyer GSTIN (optional)</Label>
                              <Input value={invBuyerGstin} onChange={(e) => { setInvBuyerGstin(e.target.value.toUpperCase()); invResetResults(); }} placeholder="e.g. 27ABCDE1234F1Z5" data-testid="input-invoice-gstin" />
                            </div>
                            <div>
                              <Label className="text-xs">Buyer state code (optional)</Label>
                              <Input value={invBuyerState} onChange={(e) => { setInvBuyerState(e.target.value); invResetResults(); }} placeholder="e.g. 27" data-testid="input-invoice-state" />
                            </div>
                          </div>

                          <div>
                            <Label className="text-xs">Notes (optional)</Label>
                            <Textarea value={invNotes} onChange={(e) => setInvNotes(e.target.value)} rows={2} data-testid="textarea-invoice-notes" />
                          </div>

                          {invPreview && (
                            <div className="rounded-lg border bg-slate-50 p-3 text-sm">
                              <p className="font-medium mb-1">GST preview</p>
                              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-gray-700">
                                <span>Supply type</span><span className="text-right capitalize">{String(invPreview.supply_type || '').replace('_', '-')}</span>
                                <span>Taxable value</span><span className="text-right">{invMoney(invPreview.taxable_paise)}</span>
                                {invPreview.supply_type === 'inter_state' ? (
                                  <><span>IGST</span><span className="text-right">{invMoney(invPreview.igst_paise)}</span></>
                                ) : invPreview.supply_type === 'intra_state' ? (
                                  <><span>CGST</span><span className="text-right">{invMoney(invPreview.cgst_paise)}</span><span>SGST</span><span className="text-right">{invMoney(invPreview.sgst_paise)}</span></>
                                ) : (
                                  <><span className="col-span-2 text-amber-600">GST not determined (missing seller/buyer state code)</span></>
                                )}
                                <span className="font-semibold">Total</span><span className="text-right font-semibold">{invMoney(invPreview.total_paise)}</span>
                              </div>
                            </div>
                          )}
                          ...
                          {invGenerated && (
                            <div className="rounded-lg border border-green-200 bg-green-50 p-3 text-sm">
                              <p className="font-medium text-green-800 flex items-center gap-1"><CheckCircle className="h-4 w-4" /> {invGenerated.invoice?.invoice_number}</p>
                              <p className="text-gray-700 mt-1">Total {invMoney(invGenerated.invoice?.total_paise)} · {String(invGenerated.invoice?.supply_type || '').replace('_', '-')}</p>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <DialogFooter className="gap-2">
                        {!invDisabled && (
                          <>
                            <Button variant="outline" onClick={invPreviewGst} disabled={invLoading || !invSourceId} data-testid="button-invoice-preview">
                              {invLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Calculator className="h-4 w-4" />} Preview GST
                            </Button>
                            {!invGenerated ? (
                              <Button style={{ backgroundColor: BRAND.primary }} className="text-white" onClick={invGenerate} disabled={invLoading || !invSourceId} data-testid="button-invoice-create">
                                {invLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Receipt className="h-4 w-4" />} Generate
                              </Button>
                            ) : (
                              <>
                                <Button variant="outline" onClick={invDownloadPdf} data-testid="button-invoice-download"><Download className="h-4 w-4" /> PDF</Button>
                                <Button style={{ backgroundColor: BRAND.primary }} className="text-white" onClick={invEmail} disabled={invLoading} data-testid="button-invoice-email">
                                  {invLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Email
                                </Button>
                              </>
                            )}
                          </>
